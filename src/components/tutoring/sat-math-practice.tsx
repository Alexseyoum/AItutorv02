...
export function SATMathPractice() {
  ...
  const initializeSession = async (topic: string) => {
    ...
    try {
      console.log("Initializing math practice session:", { topic });
      
      const response = await fetch("/api/sat/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ section: 'math', topic })
      });
      ...
      if (!data || !data.questions || !Array.isArray(data.questions) || data.questions.length === 0) {
        console.warn("No valid questions received for math, creating mock questions");
        const mockQuestions: SATQuestion[] = [];
        for (let i = 0; i < 10; i++) {
          mockQuestions.push({
            id: `mock-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${i}`,
            question: `What is 2 + 2? (Mock math practice question #${i + 1})`,
            choices: ["3", "4", "5", "6"],
        ...
        setTimeRemaining(TIME_LIMITS['math'] || 35 * 60);
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    initializeSession('Algebra: Linear Equations');
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading Math Practice...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-3xl font-bold text-green-400 mb-4">SAT Math Practice</h1>
          <p className="text-gray-300 mb-8">Preparing your math practice session...</p>
        </div>
      </div>
    );
  }
