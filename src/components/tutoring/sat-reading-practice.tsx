...
export function SATReadingPractice() {
  ...
  const initializeSession = async (topic: string) => {
    ...
    try {
      console.log("Initializing reading practice session:", { topic });
      
      const response = await fetch("/api/sat/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ section: 'reading', topic })
      });
      ...
      if (!data || !data.questions || !Array.isArray(data.questions) || data.questions.length === 0) {
        console.warn("No valid questions received for reading, creating mock questions");
        const mockQuestions: SATQuestion[] = [];
        for (let i = 0; i < 10; i++) {
          mockQuestions.push({
            id: `mock-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${i}`,
            question: `What is the main idea of this passage? (Mock reading practice question #${i + 1})`,
            choices: ["It discusses technology", "It explains a process", "It tells a story", "It argues a point"],
            correctAnswer: 3, // Index of "It argues a point"
        ...
        setTimeRemaining(TIME_LIMITS['reading'] || 65 * 60);
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    initializeSession('Reading Comprehension');
  }, []);
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading Reading Practice...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-3xl font-bold text-green-400 mb-4">SAT Reading Practice</h1>
          <p className="text-gray-300 mb-8">Preparing your reading practice session...</p>
        </div>
      </div>
    );
  }
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        {aiExplanation.keyPoints.map((point: string, index: number) => (
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        {aiExplanation.commonMistakes.map((mistake: string, index: number) => (
                          <li key={index} className="text-base text-gray-200">
                            {mistake}
                          </li>
                        ))}
